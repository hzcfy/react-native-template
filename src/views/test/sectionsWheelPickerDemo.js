...
const SectionsWheelPickerScreen = () => {
  ...
  const onDaysChange = useCallback((item) => {
    setSelectedDays(item)
  }, [])

  const onHoursChange = useCallback((item) => {
    setSelectedHours(item)
  }, [])

  const onMinutesChange = useCallback((item) => {
  ...
  const updateDisableRTLValue = useCallback((value) => {
    setDisableRTL(value)
  }, [])
